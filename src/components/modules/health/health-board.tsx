...
import { Plus, ChevronLeft, ChevronRight, Sunrise, Sun, Sunset, Cookie, type LucideIcon } from "lucide-react";
...
export function HealthBoard({ meals, today }: { meals: Meal[]; today: string }) {
  ...
  const week = useMemo(() => {
    return weekDays(selected).map((day) => {
      const dayTotal = calorieTotal(meals.filter((m) => dateToDay(m.date) === day));
      return { day, total: dayTotal };
    });
  }, [meals, selected]);

  const weekMax = Math.max(1, ...week.map((d) => d.total ?? 0));
  const weekTotal = calorieTotal(week.filter((d) => d.total != null).map((d) => ({ calories: d.total })));
  ...
  const modalType: MealType = creating ?? (editing?.type as MealType) ?? "breakfast";
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon-sm" onClick={() => setSelected(shiftDay(selected, -1))}>
            <ChevronLeft />
          </Button>
          <div className="flex flex-col items-center">
            ...
            <span className="text-fg-3 text-xs">{format(new Date(`${selected}T00:00:00`), "EEEE")}</span>
          </div>
          <Button variant="ghost" size="icon-sm" onClick={() => setSelected(shiftDay(selected, 1))}>
            <ChevronRight />
          </Button>
      ...
      <div className="bg-surface rounded-[var(--r)] border p-5 shadow-[var(--shadow-card)]">
        <div className="flex items-center justify-between">
          ...
          <span className="text-fg-3 font-mono text-xs tabular-nums">{kcal(weekTotal)}</span>
        </div>
        <div className="mt-4 flex items-end justify-between gap-2">
          {week.map((d) => {
            const isSelected = d.day === selected;
            const hasData = d.total != null;
            const height = hasData ? Math.max(6, (d.total! / weekMax) * 72) : 4;
            return (
              <button
                key={d.day}
                type="button"
              ...
              >
                <div className="flex h-[72px] w-full items-end justify-center">
                  <div
                    className={cn("w-2 rounded-full", !hasData && "bg-surface-3")}
                    style={{
                      height: `${height}px`,
                      ...(hasData
                        ? { background: "var(--chart-1)", opacity: isSelected ? 1 : 0.55 }
                        : {}),
                    }}
                  />
                </div>
                <span className={cn("text-xs", isSelected ? "text-fg-2" : "text-fg-4")}>
      ...
      <div className="space-y-5">
          ...
          return (
            <div key={type} className="space-y-2">
              <div className="flex items-center justify-between px-1">
                ...
                <span className="text-fg-3 font-mono text-xs tabular-nums">{kcal(subtotal)}</span>
              </div>
              ...
              {rows.length === 0 ? (
                <div className="bg-surface flex items-center justify-between rounded-[var(--r)] border px-4 py-3">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setCreating(type)}>
                    <Plus /> Add
                  </Button>
                </div>
              ) : (
                <div className="bg-surface divide-border overflow-hidden rounded-[var(--r)] border [&>*]:border-t [&>*:first-child]:border-t-0">
                  {rows.map((meal) => (
                    ...
                    >
                      ...
                      <span className="text-fg min-w-0 flex-1 truncate text-[15px]">{meal.name}</span>
                      <span className="shrink-0 font-mono text-sm tabular-nums">
                        {meal.calories == null ? (
                          <span className="text-fg-4">—</span>
                        ) : (
                          <span className="text-fg-2">
                            {String(meal.calories).replace(/\B(?=(\d{3})+(?!\d))/g, " ")}
        ...
        <div className="flex items-center justify-between px-1 pt-1">
          ...
          <span className="text-fg font-mono text-base tabular-nums">{kcal(dayTotal)}</span>
        </div>
      </div>
