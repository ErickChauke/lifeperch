...
import { addNoteAttachment, deleteNoteAttachment } from "@/actions/notes";
...
// Manages a note's file attachments from the read view. Holds its own list state
// seeded from the server so a freshly added or removed row shows immediately
// without refetching the parent note.
export function NoteAttachments({
  ...
  async function onPick(file: File | null) {
    ...
    setProgress(0);
    try {
      const result = await uploadFile(file, "lifeperch/notes", setProgress);
      const row = await addNoteAttachment(noteId, { name: file.name, ...result });
